...
# Hardcoded AES-128-GCM key used by Go2 firmware >= 1.1.8 to encrypt the
# con_notify "data1" field when "data2" == 2 (protocol v2).
_CON_NOTIFY_AES_KEY = bytes(
    [232, 86, 130, 189, 22, 84, 155, 0, 142, 4, 166, 104, 43, 179, 235, 227]
)


def _decrypt_con_notify_data1(encrypted_b64: str) -> str:
    """Decrypt the data1 field from con_notify when data2 == 2 (firmware >= 1.1.8)."""
    ...
    nonce = data[-28:-16]
    ciphertext = data[:-28]
    aesgcm = AESGCM(_CON_NOTIFY_AES_KEY)
...
def send_sdp_to_local_peer_new_method(ip, sdp):
    try:
        ...
        if response:
            ...
            # Firmware >= 1.1.8 encrypts data1 with AES-GCM when data2 == 2
            if data2 == 2:
                data1 = _decrypt_con_notify_data1(data1)
